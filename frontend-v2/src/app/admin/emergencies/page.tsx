...
import { Radio, Clock, MapPin, CheckCircle2, XCircle, AlertTriangle } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-center gap-4 p-3 rounded-lg bg-[#161b22] border border-[#21262d] text-xs font-mono">
        <span className="text-ok-400 flex items-center gap-1.5">
          ...
          {cases.filter(c => c.status === 'BROADCASTING').length} Broadcast Active
        </span>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {cases.map(c => (
            <div key={c.id} className={`v2-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all ${
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-mono text-[#484f58]">
                    ...
                    <MapPin size={10} /> {c.address || 'Bengaluru'}
                  </div>
